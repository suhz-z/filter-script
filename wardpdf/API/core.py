...
def ocr_malayalam(pixmap):
    """Run OCR on image pixmap with high-contrast bottom line (gender/age) pass."""
    img = Image.open(io.BytesIO(pixmap.tobytes("png"))).convert("L")

    # Strong binarization for better text edges
    img = img.point(lambda x: 0 if x < 165 else 255, "1")

    # Full-page OCR
    text = pytesseract.image_to_string(img, lang="mal+eng", config="--oem 3 --psm 6")
    lines = [line.strip() for line in text.splitlines() if line.strip()]

    # Re-OCR top ~18% for clean Voter ID (English only)
    if lines:
    ...
    # Re-OCR bottom 40% (gender/age area)
    bottom_crop = img.crop((0, int(img.height * 0.6), img.width, img.height))
    bottom_text = pytesseract.image_to_string(bottom_crop, lang="mal+eng", config="--psm 6")
    bottom_text = re.sub(r"[|•■□]+", "", bottom_text).strip()

    if bottom_text:
        bottom_lines = [b.strip() for b in bottom_text.splitlines() if b.strip()]
        last_line = " ".join(bottom_lines[-2:]) if len(bottom_lines) >= 2 else bottom_lines[-1]
        if last_line:
            if len(lines) > 1:
                lines[-1] = last_line
            else:
...
def parse_voter_lines(lines):
    """Parse OCR lines into structured fields with translated Gender/Age (M/F)."""
    ...
    # Focus on bottom (6th) line
    bottom_line = lines[-1].lower()
    bottom_line = re.sub(r"[^\w\d/|:]+", "", bottom_line)
    bottom_line = bottom_line.replace("|", "/").replace(":", "/").replace("l", "/").replace("i", "/")

    # Malayalam -> English mapping
    gender_map = {
        "പു": "M", "പുരുഷ": "M", "ആൺ": "M",
        "സ്ത്രീ": "F", "പെണ്": "F", "പെൺ": "F"
    }
    ...
    # Extract numeric age
    age_match = re.search(r"(\d{1,3})", bottom_line)
    age = age_match.group(1) if age_match else ""

    if gender or age:
        gender_age = f"{gender}/{age}".strip("/")

    # Optional: house name (5th line)
    ...
    for page_idx in tqdm(range(1, len(doc)), desc="📄 Processing pages", unit="page"):
        ...
        for rect_index, r in enumerate(rects):
            try:
                ...
                voter_id, name, relation, house_no, house_name, gender_age = parse_voter_lines(lines)
